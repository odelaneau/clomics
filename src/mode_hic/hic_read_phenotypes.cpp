...
void hic_data::scanPhenotypes(string fbed) {
	int n_includedP = 0;
	int n_excludedP = 0;
	...
	if (regionPhenotype.chr != "NA"){
		...
		while (tbx_itr_next(fp, tbx, itr, &str) >= 0) {
			...
			if (filter_phenotype.check(tokens[3])) {
				phenotype_vec.push_back(hic_phenotype_data(tokens[3], tokens[0], atoi(tokens[1].c_str()) + 1, atoi(tokens[2].c_str())));
				n_includedP++;
			} else n_excludedP ++;
		}
		tbx_itr_destroy(itr);
...
void hic_data::sortPhenotypes() {
	...
	phenotype_hic_val = vector < vector < float > > (phenotype_count, vector < float > (phenotype_count, 0));
	phenotype_hic_cnt = vector < vector < short > > (phenotype_count, vector < short > (phenotype_count, 0));
	...
	vrb.title("Building interval tree for phenotype data");
	vector < Interval < int > > interval_vec;
	for (int p = 0 ; p < phenotype_start.size() ; p ++) interval_vec.push_back(Interval < int > (phenotype_start[p], phenotype_end[p], p));
	phenotype_tree = IntervalTree < int > (interval_vec);
	vrb.bullet("done!");
}


void hic_data::readPhenotypes(string fbed) {
	int n_includedS = 0;
	int n_includedP = 0;
	int n_excludedP = 0;

...
void call_data::readTree(string ftxt) {
	string buffer;
	...
	while (getline(fd1, buffer)) {
		...
		ccluster * new_cluster = new ccluster (tokens[3], index, count);
		new_cluster->acorrelation = atof(tokens[5].c_str());
		new_cluster->apvalue = atof(tokens[6].c_str());
		new_cluster->bcorrelation = atof(tokens[7].c_str());
		new_cluster->bpvalue = atof(tokens[8].c_str());
		new_cluster->ccorrelation = atof(tokens[9].c_str());
		new_cluster->cpvalue = atof(tokens[10].c_str());
		new_cluster->dcorrelation = atof(tokens[11].c_str());
		new_cluster->dpvalue = atof(tokens[12].c_str());
		new_cluster->ecorrelation = atof(tokens[13].c_str());
		new_cluster->epvalue = atof(tokens[14].c_str());
		new_cluster->pcavariance = atof(tokens[15].c_str());

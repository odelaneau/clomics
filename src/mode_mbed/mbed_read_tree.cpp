...
void mbed_data::readTree(string ftxt) {
	string buffer;
	...
	while (getline(fd1, buffer)) {
		...
		mcluster * new_cluster = new mcluster (tokens[3], index, count);
		new_cluster->start = atoi(tokens[17].c_str());
		new_cluster->end = atoi(tokens[18].c_str());

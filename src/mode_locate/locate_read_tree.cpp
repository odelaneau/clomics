...
void locate_data::readTree(string ftxt) {
	string buffer;
	...
	while (getline(fd1, buffer)) {
		...
		lcluster * new_cluster = new lcluster (tokens[3], index, count);
	...
	while (getline(fd2, buffer)) {
		...
		if (tokens[1] != "NA" || tokens[2] != "NA") {
			...
			map_leaves.insert(pair < string, lcluster * > (vec_nodes[index_pp]->sid, vec_nodes[index_pp]));
			n_leaves ++;
		}
	}
	tree = vec_nodes.back();

...
void corr_data::scanPhenotypes(string fbed) {
	int n_includedP = 0;
	int n_excludedP = 0;
	...
	if (regionPhenotype.chr != "NA"){
		...
		while (tbx_itr_next(fp, tbx, itr, &str) >= 0) {
			...
			if (filter_phenotype.check(tokens[3])) {
				phenotype_vec.push_back(corr_phenotype_data(tokens[3], tokens[0], atoi(tokens[1].c_str()) + 1, atoi(tokens[2].c_str())));
				n_includedP++;
			} else n_excludedP ++;
		}
		tbx_itr_destroy(itr);
...
void corr_data::readPhenotypes(string fbed) {
	int n_includedS = 0;
	int n_includedP = 0;
	int n_excludedP = 0;

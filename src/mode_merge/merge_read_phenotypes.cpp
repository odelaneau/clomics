...
void merge_data::scanPhenotypes(string fbed) {
	int n_includedP = 0;
	int n_excludedP = 0;
	...
	while (hts_getline(fp, KS_SEP_LINE, &str) >= 0) {
		...
		if (str.l && str.s[0] != tbx->conf.meta_char) {
			...
			if (filter_phenotype.check(tokens[3])) {
				phenotype_vec.push_back(merge_phenotype_data(tokens[3], tokens[0], atoi(tokens[1].c_str()) + 1, atoi(tokens[2].c_str())));
				n_includedP++;
			} else n_excludedP ++;
		}
	}
...
void merge_data::sortPhenotypes() {
	...
	phenotype_assay = vector < int > (phenotype_count, -1);
	phenotype_grp = vector < vector < int > > (phenotype_count, vector < int > (1, -1));
	vrb.bullet("#samples = " + stb.str(sample_count) + " #phenotypes = " + stb.str(phenotype_count));
}

void merge_data::readPhenotypes(string fbed, int idx) {
	int n_includedS = 0;
	int n_includedP = 0;
	int n_excludedP = 0;
	...
	while (hts_getline(fp, KS_SEP_LINE, &str) >= 0) {
		...
		if (str.l && str.s[0] != tbx->conf.meta_char) {
			...
			if (it_map != phenotype_map.end()) {
				...
				phenotype_assay[it_map->second] = idx;
				phenotype_grp[it_map->second].back() = it_map->second;
